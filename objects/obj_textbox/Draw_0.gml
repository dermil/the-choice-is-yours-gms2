...
textbox_y = camera_get_view_y(view_camera[0]) + camera_get_view_height(view_camera[0])/3;
...
if (draw_char == text_length[page] && page == page_number - 1) {
		...
		for (var op = 0; op < option_number; op++) {
			...
			if (option_pos == op) {
				...
				draw_sprite(spr_selector, 0, (_textbox_x ) * (option_number + _op_space ) + (op * 64), _textbox_y - _op_space)	
			}
			
			//draw choices text
			draw_text(((_textbox_x ) * (option_number + _op_space) + (op * 64) ), _textbox_y - _op_space, option[op])
		}
}
